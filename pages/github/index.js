    ...
    repodata.repositories.forEach((repo) => {
        ...
        repo.contributors.forEach((contributor) => {
            if (!contributorColors[contributor.username]) {
                contributorColors[contributor.username] = (contributor.username === 'AshaniLiyanagamage') ? 'yellow' : getRandomColor(0.6);
            }

            contributor.commit_details.forEach((commit) => {
                const commitDate = new Date(commit.commit_data).toISOString().split('T')[0];
        ...
        datasetsArray.push({
                    ...
                    tooltip: {
                        callbacks: {
                            label: function (context) {
                                ...
                                const commitDetails = data.commitDetails.map((commit) => `* ${commit}`);
                                return [`Commits: ${data.y}`, '', 'Commit Details:', ...commitDetails];
                            },
                        },
                        borderWidth: 0.5,
                        bodyFont: {
            ...
            datasets: Object.values(contributorsData), contributorColors
        });
    });
