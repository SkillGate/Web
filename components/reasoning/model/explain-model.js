...
const ExplainPopup = ({ onClose, title, explanableData }) => {
  const [loading, setLoading] = useState(false);

  const formatChartData = (responseArray) => {
    const labels = responseArray?.map(([label]) => label);
    const dataPoints = responseArray?.map(([, value]) => value);
    return {
      labels,
      datasets: [
        {
  ...
  const data = formatChartData(explanableData?.prediction?.response?.company);
  const data2 = formatChartData(explanableData?.prediction?.response?.candidate);

  console.log(data);
  console.log(data2);
  console.log(explanableData);
